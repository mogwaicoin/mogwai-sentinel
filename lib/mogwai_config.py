...
class MogwaiConfig():
    ...
    def slurp_config_file(self, filename):
        # read mogwai.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is mogwai.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # standard Mogwai defaults...
        default_port = 17710 if (network == 'mainnet') else 17810

        # use default port for network if not specified in mogwai.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from mogwai.conf
