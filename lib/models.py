...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, mogwaid):
        golist = mogwaid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_mogwaid(mogwaid, item)
    ...
    @classmethod
    def import_gobject_from_mogwaid(self, mogwaid, rec):
        import decimal
        import mogwailib
        import inflection
        ...
        # shim/mogwaid conversion
        object_hex = mogwailib.SHIM_deserialise_from_mogwaid(object_hex)
        objects = mogwailib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from mogwaid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from mogwaid, with every run
        ...
        # get/create, then sync payment amounts, etc. from mogwaid - Mogwaid is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(mogwaid)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from mogwaid! %s" % e)
            govobj.vote_delete(mogwaid)
            return (govobj, None)
    ...
    def vote_delete(self, mogwaid):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(mogwaid, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, mogwaid, signal, outcome):
        import mogwailib
        ...
        output = mogwaid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = mogwailib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(mogwaid, signal)

    def sync_network_vote(self, mogwaid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = mogwaid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 mogwai addr, non-multisig
            if not mogwailib.is_valid_mogwai_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Mogwai address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (mogwailib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MogwaiDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import mogwailib
        obj_data = mogwailib.SHIM_serialise_for_mogwaid(self.serialise())
    ...
    def prepare(self, mogwaid):
        try:
            object_hash = mogwaid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not mogwailib.is_valid_mogwai_address(addr, config.network):
    ...
    def hash(self):
        import mogwailib
        return mogwailib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, mogwaid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - mogwaid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, mogwaid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - mogwaid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, mogwaid):
        now = int(time.time())
        return (self.created_at < (now - mogwaid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, mogwaid):
        if self.is_expired(mogwaid):
            return False

        return True

    def is_deletable(self, mogwaid):
        if self.is_expired(mogwaid):
            return True
