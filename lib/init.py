...
def has_mogwai_conf():
    ...
    valid_mogwai_conf = False

    # ensure mogwai_conf exists & readable
    #
    # if not, print a message stating that Mogwai Core must be installed and
    # configured, including JSONRPC access in mogwai.conf
    try:
        f = io.open(config.mogwai_conf)
        valid_mogwai_conf = True
    except IOError as e:
        print(e)

    return valid_mogwai_conf
...
def main():
    ...
    if not has_mogwai_conf():
        print("MogwaiCore must be installed and configured, including JSONRPC access in mogwai.conf")
        sys.exit(1)
