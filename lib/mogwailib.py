...
def is_valid_mogwai_address(address, network='mainnet'):
    ...
    mogwai_version = 127 if network == 'testnet' else 50
    ...
    try:
        ...
        # rescue from exception, not a valid Mogwai address
        return False

    if (address_version != mogwai_version):
        return False
...
# shims 'til we can fix the mogwaid side
def SHIM_serialise_for_mogwaid(sentinel_hex):
    ...
    # superblock => "trigger" in mogwaid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # mogwaid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    mogwaid_hex = serialise(obj)
    return mogwaid_hex


# shims 'til we can fix the mogwaid side
def SHIM_deserialise_from_mogwaid(mogwaid_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(mogwaid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('mogwai.conf')
