...
class MogwaiDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some mogwaid methods
    ...
    @classmethod
    def from_mogwai_conf(self, mogwai_dot_conf):
        from mogwai_config import MogwaiConfig
        config_text = MogwaiConfig.slurp_config_file(mogwai_dot_conf)
        creds = MogwaiConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = mogwailib.parse_raw_votes(raw_votes)

        return self.gobject_votes[object_hash]

    def is_govobj_maturity_phase(self):
        # 3-day period for govobj maturity
        maturity_phase_delta = 2160      # ~(60*24*3)/2.6
    ...
    def we_are_the_winner(self):
        ...
        winner = mogwailib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = mogwailib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_mogwaid_proto_version_with_sentinel_ping)
